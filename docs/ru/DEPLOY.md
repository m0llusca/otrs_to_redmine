...
Письма и заметки тикета **не** уходят в Redmine после создания/привязки. При привязке/отвязке — только короткая заметка (ссылка на тикет + агент), без штампа моста. Redmine → OTRS нужен Daemon и `Redmine::InboundSync`.
